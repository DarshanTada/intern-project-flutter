...
class FirestoreService {
  ...
  Stream<List<Game>> getTodaysGamesStream() {
    ...
    debugPrint('Querying today\'s games: $startStr to $endStr');
          ...
          final games = snapshot.docs
              .map((doc) {
                try {
                  ...
                  debugPrint(
                    'Parsed game: ${game.gameId} - ${game.awayTeam.name} @ ${game.homeTeam.name}',
                  );
                  return game;
                } catch (e) {
  ...
  /// Includes games from yesterday (for final), today, and tomorrow (to catch games starting at midnight UTC)
  Stream<List<Game>> getGamesByStatusStream(String status) {
    ...
    // Fetch games from yesterday (final), today (scheduled/live), and tomorrow
    // Filter by status in memory to avoid composite index requirement
    return _firestore
        .collection('games')
        .where('startTime', isGreaterThanOrEqualTo: yesterday.toIso8601String())
  ...
  Stream<List<Game>> getTeamGamesStream(int teamId, {int limit = 5}) {
    ...
    final cutoffDate = DateTime.now().toUtc().subtract(
  ...
  Future<List<Game>> getTeamAllGames(int teamId) async {
    try {
      ...
      final allGames = <Game>[];
      allGames.addAll(
        homeGames.docs.map((doc) => Game.fromFirestore(doc.data(), doc.id)),
      );
      allGames.addAll(
        awayGames.docs.map((doc) => Game.fromFirestore(doc.data(), doc.id)),
      );
